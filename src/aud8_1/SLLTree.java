...
public class SLLTree<E> implements Tree<E> {
	...
	@Override
	public Iterator<Node<E>> childrenNodes(Node<E> node) {
        return new NodeIterator(((SLLNode<E>) node).firstChild);
	}

	private class NodeIterator implements Iterator<Node<E>> {
		private final SLLNode<E> node;

		public NodeIterator(SLLNode<E> node) {
			this.node = node;
		}
